...
from chatbot_frontend.graph.graph_state import SessionState
from chatbot_frontend.components.images import upload_image, assess_image
from chatbot_frontend.chat.recommend import display_personalized_rank, display_interaction_flow
...
def main():
    ...
    if uploaded_image != st.session_state.uploaded_image and assess_image(uploaded_image):
        display_personalized_rank(user_id, uploaded_image, ITEMS_TO_SHOW)
    ...
    if user_input := st.chat_input():
        ...
        campaign = display_interaction_flow(st.session_state.state["messages"], st.session_state.state["current_filter"])



        st.write(st.session_state.state["current_filter"])

    if st.session_state.recommended_items:
        save_chat_logs(user_id, st.session_state.timestamp, remove_image_from_history(st.session_state.state["messages"]),
                       st.session_state.recommended_items)
